...
|---|---|---|
| `aggregate-squash` | The neuron, or something a bias fold would touch, uses an aggregate squash (`IF`, `MEAN`, `MINIMUM`, …) that does not sum its inputs. Since #135 a synapse visit into an aggregate target is counted here: the bias fold is not a sum. | **Yes, since #103** — constant substitution. The code is still recorded where neither the core prune nor a substitution could be built; since Issue #199 the absent-statistic half of that population is gone, because an unmeasured visit no longer stops at any rung. |
