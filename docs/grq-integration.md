...
learnings/passes/<host>.jsonl                           completed-sweep markers, general population
...
learnings-team-<island>/passes/<host>.jsonl             completed-sweep markers, one island
```
...
Until Issue #171 that second stamp fired **only** after a coverage tail, so a
run whose accept came from the ordinary search rebuilt its sweep, kept screening
and still published the figure at the cut: one commit reported
`sweep 10338/55649 (18.6%)` in its subject and `13481/55649 (24.2%)` in the
`coverage.txt` block beside it. The re-stamp is now unconditional on any accept
(it is skipped only without `--learnings-dir`, where there is no coverage to
carry), and the subject clause is rendered by `Coverage::subject_clause` — the
same value `coverage.txt` and `coverage.json` are rendered from. The subject's
`sweep X/Y` therefore agrees with the block GRQ relays beside it, figure for
figure, and the block names the measurement both came from on its `snapshot:`
line. **GRQ needs no change**: it already uses the tag verbatim and relays
`coverage.txt` verbatim, so it consumes the canonical report rather than
...
| `<output-dir>/coverage.txt` | `grq_ockham_read_coverage` | Line-oriented block relayed verbatim as the commit description. Since Issue #59 it may carry `winners:` / `bundles:` / `dropped:` lines after the coverage lines; each is omitted when it has nothing to report, and a run that screened nothing renders the coverage lines alone. Since Issue #74 the tagged line reads `tagged:    N carry tags, screened like any other` — it replaced the `skipped:` line, and is still omitted when no neuron is tagged. Issue #87 removed the `declared:` line that followed it. Issue #93 added a `blocked:   N checked with no cut proposed` line before the tagged line — omitted when nothing is blocked — and the `progress:` line now reads `newly checked` rather than `newly screened`. Issue #100 added an `epoch:     corpus <identity> — coverage counts this corpus only` line naming the corpus the figures were measured against. Issue #102 made the block epoch-aware throughout: the first line is now `sweep:     N of M hidden (Z% of epoch)`, the `epoch:` line moved directly under it and carries the first eight characters of the identity, `unchecked:` reads `N remaining this epoch` — or `0 remaining — sweep complete for this epoch` when the sweep has finished — and a `history:   N of M ever checked across K corpus epochs` line after `progress:` reports cumulative coverage, omitted when the store holds no records. Issue #103 added a `reasons:   <code> N (P%) · …` line directly under `blocked:`, splitting that total by blocked-reason code and omitted with it. Issue #140 inserted two lines between `progress:` and `history:`, so the order is now `progress:` → `passes:` → `visits:` → `history:`: `passes:    N complete this epoch · M this run · pass K in progress` is never omitted and is the line that keeps moving once the unique percentage reaches 100%, and `visits:    N hidden neurons visited this run · K revisited` follows it, omitted when the run reached nothing. Issue #162 appended a throughput block after the winner lines: one `funnel:    <kind> N visits · N blocked · N judged · N proposed · N screened · N scored` line per kind — the synapse line omitted when the run walked no edge — then `rate:      neurons N screened/h · synapses N screened/h · full rescan ~Xh` and `eta:       visit rescan ~Xh · scored rescan ~Xh · N neurons + N edges eligible`. The whole block is omitted when the run measured no wall clock or reached no visit, an ETA nothing was measured for renders `unknown` rather than a zero, and one with nothing proposable left renders `none left`. Issue #171 inserted one line between the coverage lines and `progress:`, so the order is now `tagged:` → `snapshot:` → `progress:`: `snapshot:  final · creature <id> · N hidden + M synapses = K visits` names the single measurement every figure above it came from — the stage, the creature the counts were taken over, and the two populations that add up to the `sweep:` denominator. It is omitted only by an artefact written before it existed. Absent or blank is a supported no-op. |
| `<output-dir>/coverage.json` | *nothing in GRQ today* | Written by Ockham; GRQ reads only `coverage.txt`. Since Issue #59 it carries an additive `winners` object beside the existing coverage fields. Issue #74 changed no key: `checkable` now counts **every** hidden neuron, tagged ones included, so the percentage no longer overstates progress. Issue #93 added an additive `blocked` key — the checked UUIDs no cut was proposed for — and a pre-#93 file still deserialises. Issue #100 added an additive `corpusIdentity` key naming the screening epoch — in full, however short the prose form is — and a pre-#100 file still deserialises. Issue #102 added an additive `history` object (`checkedEver`, `epochs`) carrying cumulative coverage across every epoch, kept out of the current-epoch percentage; a pre-#102 file still deserialises. Issue #103 added an additive `blockedByReason` object with one fixed key per reason code (`aggregateSquash`, `missingActivation`, `unsafeTopology`, `validationFailed`, `noOutputPath`, `other`, `unrecorded`) whose counts sum to `blocked`; a pre-#103 file still deserialises, as no reasons. Issue #140 added an additive `passes` object (`sweepRestartsRun`, `sweepsCompletedEpoch`, `currentPass`, `visitedRun`, `revisitedRun`) reporting complete sweeps over the epoch beside the unique coverage figures, never inside them; a pre-#140 file still deserialises. Issue #162 added an additive `throughput` object (`elapsedMs`, `funnel`, `hidden`, `synapses`, `visitsPerHour`, `proposedPerHour`, `screenedPerHour`, `fullScoredPerHour`, `proposableEstimate`, `visitRescanHours`, `scoredRescanHours`) carrying the screening funnel — `visits`, `revisits`, `blocked`, `judged`, `proposed`, `sampleScreened`, `sampleWinners`, `fullScored`, `confirmed`, `applied`, each split into `neurons` / `synapses` / `total` — with per-hour rates over the run's measured wall clock and both rescan ETAs, each computed per kind and summed; an unmeasurable ETA — and an unmeasurable `proposableEstimate` — is `null`, and a pre-#162 file still deserialises. Issue #171 added an additive `snapshot` object (`stage`, `creature`) naming the one measurement every figure in the file came from — `stage` is always `"final"`, and `creature` is the checksum of the creature the counts were taken over, the same creature `best.json` carries; a pre-#171 file still deserialises, as no snapshot. |
