...
- **violation** — test journal readers used `filter_map(… .ok())`, so a broken serialisation would look like a missing record — evidence: `ockham/src/run.rs`, `ockham/tests/sweep_restart.rs` — reason: fixed, an unparsable line now fails the test
