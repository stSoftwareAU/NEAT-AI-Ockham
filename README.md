...
Every `uncompensated` row on the telemetry detail carries `droppedMean` beside
its reason — the magnitude `weight x mean` of the term the target lost, where
the statistic Ockham measured or the creature's own structure proves one. A
`no-statistics` row proves neither, so the field is absent rather than `0`: a
zero would read as "nothing was lost".
...
stopped aggregating rather than leaving a reader to diff two creatures. `blocked` never meant *not pruneable forever*: it means the
