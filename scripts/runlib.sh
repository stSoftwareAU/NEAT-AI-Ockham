#!/usr/bin/env bash
# Build, install, and clean neat_ai_ockham for the fleet worker (#219).
#
# Installs the crate CLI only:
#   ~/.cargo/bin/neat_ai_ockham
# stamped with .neat_ai_ockham.version beside it.
#
# stdout is the CLI binary path only. Diagnostics go to stderr.
# A second run whose stamp matches prints
#   [neat_ai_ockham] already installed v<x>
...
# NEAT-AI-core#680 / this repo #209. This ships the install contract so the
# GRQ fleet can run ockham stages (stSoftwareAU/GRQ#4796).
#
# Cross-platform: macOS bash 3.2, Ubuntu, AWS Linux.
set -euo pipefail

CRATE="neat_ai_ockham"
BIN_NAME="neat_ai_ockham"
...
ensure_installed() {
  ...
  if _already_installed "${desired}" "${bin}" "${mark}"; then
    echo "[neat_ai_ockham] already installed v${desired}" >&2
    echo "${bin}"
    return 0
  fi
