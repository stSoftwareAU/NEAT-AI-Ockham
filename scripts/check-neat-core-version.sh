...
#
# Ockham consumes neat-core through an UNPINNED `path` dependency that always
# tracks head (see `ockham/Cargo.toml`). The safeguard is this CI gate: it
# fails when the sibling neat-core presents a breaking bump Ockham has not yet
# acknowledged, forcing a deliberate upgrade instead of tracking head blindly.
#
# Mechanism — version-baseline check:
#   * Ockham records the last-handled neat-core version in the checked-in
...
#
# "Handling" a breaking bump = a deliberate Ockham PR that makes the
...
remediation() {
  cat >&2 <<EOF
       neat-core has presented a breaking bump Ockham has not handled.
       To clear this gate, in a single deliberate PR:
         1. Update ockham for the breaking neat-core change.
